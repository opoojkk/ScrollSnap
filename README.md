# 📸 ScrollSnap - 浏览器滚动长图截图工具

ScrollSnap 是一个强大的 Chrome 浏览器扩展，允许你选择页面上的任意区域，在滚动时自动捕获并**智能拼接成一张长图**。

## ✨ 功能特性

- 🎯 **自由选择区域**: 在页面上拖动鼠标，选择任意矩形区域
- 📸 **自动捕获**: 页面滚动时自动捕获选定区域的每一帧
- 🔗 **智能拼接**: 根据滚动位置自动拼接成一张完整的长图
- 💾 **多格式导出**: 支持 PNG/JPEG/WebP 三种格式，满足不同需求
- 📏 **高质量渲染**: Canvas 高质量渲染，文字和图像清晰
- 🎨 **直观界面**: 简洁美观的用户界面
...
3. 点击"**下载长图**"按钮
4. 插件会自动将所有画面拼接成一张长图
5. 拼接完成后会自动下载到你的默认下载文件夹
6. 文件名格式：`scrollsnap_long_时间戳.png/jpg/webp`
...
3. **智能拼接** (`content.js` - `stitchImages()`)
   - 按滚动位置排序所有画面
   - 计算帧与帧之间的滚动距离
   - 使用 Canvas 精确拼接图片
   - 生成完整的长图
...
- [x] ~~添加截图拼接功能（自动合并为长图）~~ ✅ 已实现
- [x] ~~支持多格式导出（PNG/JPEG/WebP）~~ ✅ 已实现
- [ ] 支持区域大小调整和移动
- [ ] 添加截图历史记录
- [ ] 支持导出为 PDF
- [ ] 添加快捷键支持
